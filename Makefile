...
demo:
	...
	# (margin fill, window bar, terminal background) into visible banding.
	docker run --rm --entrypoint gifsicle -v "$(CURDIR)/scripts/demo/out:/out" vev-demo -O3 --lossy=30 -o /out/demo.gif /out/demo.gif
	mkdir -p docs/assets
