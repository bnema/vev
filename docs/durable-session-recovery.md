...
- `vev cmd -s NAME session-recovery export PATH`
- `vev cmd -s NAME session-recovery discard`

Retry validates persisted state without replacing it. Restore validates and promotes the selected catalogue fallback. Export writes recoverable data without mutation. Discard creates a new incarnation and retains the old record and snapshots under `quarantine/` until an explicit later purge.
