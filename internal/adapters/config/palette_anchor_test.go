...
func TestParsePaletteAnchor(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "default is auto", want: domain.PaletteConfig{}},
		...
		{name: "invalid first preserves auto", input: "palette.anchor = diagonal\n", want: domain.PaletteConfig{}, warnings: []domain.Warning{{Line: 1, Msg: "invalid palette.anchor \"diagonal\""}}},
...
func TestLoadMissingPaletteAnchorDefaultsToAuto(t *testing.T) {
	t.Parallel()
	cfg, warnings, err := Load(filepath.Join(t.TempDir(), "missing"))
	require.NoError(t, err)
	require.Empty(t, warnings)
	require.Equal(t, domain.PaletteConfig{}, cfg.Palette)
}

func TestWatchPaletteAnchorRestoresAutoAfterFileRemoval(t *testing.T) {
	...
	clk.advance(2 * time.Second)
	got := <-changes
	require.Equal(t, domain.PaletteConfig{}, got.Palette)
	cancel()
