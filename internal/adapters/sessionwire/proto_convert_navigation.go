...
func pickerControlRequestToWire(message protocol.PickerControlRequest) (*wire.PickerControlRequest, error) {
	if err := protocol.ValidatePickerControlRequest(message); err != nil {
		return nil, err
	}
	out := &wire.PickerControlRequest{
		Version:        uint32(message.Version),
		RequestId:      message.RequestID,
		Operation:      uint32(message.Operation),
	...
	for _, target := range message.Targets {
		target := target
		out.Targets = append(out.Targets, exactTargetToWire(&target))
	}
	return out, nil
}

func pickerControlRequestFromWire(message *wire.PickerControlRequest) (protocol.PickerControlRequest, error) {
	...
	version, err := mustUint16(message.GetVersion())
	if err != nil {
		return protocol.PickerControlRequest{}, protocol.ErrInvalidNavigation
	}
	request.Version = version
	request.RequestID = message.GetRequestId()
	request.Operation, err = enum8[protocol.PickerControlOperation](message.GetOperation())
	if err != nil {
		return protocol.PickerControlRequest{}, protocol.ErrInvalidNavigation
	}
	request.SourceRevision = message.GetSourceRevision()
	request.SourceID = message.GetSourceId()
	request.Key = message.GetKey()
	for _, target := range message.GetTargets() {
		converted, err := exactTargetFromWire(target)
		if err != nil || converted == nil {
			return protocol.PickerControlRequest{}, protocol.ErrInvalidNavigation
		}
		request.Targets = append(request.Targets, *converted)
	}
	if err := protocol.ValidatePickerControlRequest(request); err != nil {
		return protocol.PickerControlRequest{}, err
	}
	return request, nil
}
