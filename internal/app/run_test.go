...
func TestParseArgs(t *testing.T) {
	tests := []struct {
		...
		{name: "hidden daemon stop", args: []string{"_daemon-stop"}, wantKind: kindDaemonStop},
		{name: "hidden daemon stop rejects args", args: []string{"_daemon-stop", "x"}, wantErr: true},
		{name: "kill unknown flag", args: []string{"kill", "-x"}, wantErr: true},
