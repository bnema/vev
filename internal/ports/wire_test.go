...
func TestHelloGoldenAndRoundTrip(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			want: []byte{0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
		},
	}
...
func TestHelloOutputWindowByteExactValues(t *testing.T) {
	for _, window := range []uint8{0, 1, 8} {
		t.Run(fmt.Sprintf("window_%d", window), func(t *testing.T) {
			...
			// Every preceding field in this deliberately empty Hello has a fixed,
			// byte-exact zero encoding; the final byte is the negotiated window.
			want := append(make([]byte, 38), window)
			want[1] = 14
