...
func Run(ctx context.Context, dialer ports.Dialer, term ports.Terminal, intent uint8, name string, log *slog.Logger) (retErr error) {
	if log == nil {
		log = slog.Default()
	}
	ms := milestones{}
	...
	for {
		...
		result := attachOnce(ctx, transport, term, attemptIntent, name, resumeToken, processClientID, &ms, enterRaw, clearStatus, log)
		if result.welcomed {
			backoff = defaultReconnectBackoff.initial
		}
		_ = transport.Close()
...
func Attach(ctx context.Context, transport ports.Transport, term ports.Terminal, intent uint8, name string) error {
	return Run(ctx, singleTransportDialer{transport: transport}, term, intent, name, slog.Default())
}
...
func attachOnce(ctx context.Context, transport ports.Transport, term ports.Terminal, intent uint8, name string, resumeToken uint64, clientID [16]byte, ms *milestones, enterRaw func() error, clearStatus func(), log *slog.Logger) attachResult {
	...
	go runStdin(loopCtx, cancel, term.In(), sendCh, trueColor, log)
...
func runStdin(ctx context.Context, cancel context.CancelFunc, in io.Reader, out chan<- ports.Frame, trueColor bool, log *slog.Logger) {
