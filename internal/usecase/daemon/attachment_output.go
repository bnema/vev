...
func (p *preparedAttachmentOutput) send(echoAck uint64, send func(protocol.Output) error) error {
	if p == nil || p.ansi == nil {
		return nil
	}
	sendOutput := send
	if p.graphics != nil {
		sendOutput = func(output protocol.Output) error {
...
func (p *preparedOutput) send(data []byte, echoAck uint64, send func(protocol.Output) error) error {
...
func (s *attachmentOutput) sideEffectLocked(data []byte, echoAck uint64) (protocol.Output, error) {
