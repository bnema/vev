...
func TestCopyModeLifecycleClosePaneClearsRecoveredClientState(t *testing.T) {
	...
	ac.setSession(sess)
	closing.mu.Lock()
	document := scopy.NewDocument(scopy.NewSnapshot(closing.history, closing.screen, closing.screen.LineBounds(), nil), domain.DefaultWordSeparators)
	closing.mu.Unlock()
	ac.overlays.copyMu.Lock()
...
func TestCopyModeLifecycleDoesNotRenderCandidateBeforeValidation(t *testing.T) {
	...
	p := tb.focusedPane()
	p.mu.Lock()
	document := scopy.NewDocument(scopy.NewSnapshot(p.history, p.screen, p.screen.LineBounds(), nil), domain.DefaultWordSeparators)
	p.mu.Unlock()
