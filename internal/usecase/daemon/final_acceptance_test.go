...
func TestAcceptancePaletteNewTabUsesSessionGeometryAfterAttachmentResize(t *testing.T) {
	...
	secondToken := sess.captureAttachmentCapability(second, second.transport())
	secondToken.lease = rc.attachmentLease(second)
	second.installTestAttachmentCapability(secondToken)
	...
	first.sendMu.Unlock()
	secondEffect, admitted := second.beginAttachmentEffect(secondToken)
...
func TestAcceptanceAttachmentStateIsolationAcrossResetResizeAndDetach(t *testing.T) {
	...
	d.handleAttachmentClientFrame(secondToken, ports.Frame{
		...
		Payload: ports.MarshalOutputResetRequest(protocol.OutputResetRequest{}),
	})
