...
func TestFloatingInstalledPaneSharesExactOpenContext(t *testing.T) {
	pty := newContextAwareFloatingPTY()
	factory := &contextAwareFloatingFactory{pty: pty, opened: make(chan context.Context, 1)}
	d := newTestDaemon(t, factory, stubClock{})
	tb := newFloatingTestTab(t)
	...
	tb.mu.Unlock()
	require.NotNil(t, floating)
	require.Same(t, openCtx, floating.ctx, "installed pane must adopt the exact context that owns its PTY")
	require.NotNil(t, floating.cancel, "installed pane must retain a cancel function for later teardown")
...
func TestFloatingCancelClosureIsIdempotentAfterOwnershipTransfer(t *testing.T) {
	pty := newContextAwareFloatingPTY()
	factory := &contextAwareFloatingFactory{pty: pty, opened: make(chan context.Context, 1)}
	d := newTestDaemon(t, factory, stubClock{})
	tb := newFloatingTestTab(t)
	...
	// Calling the pane's cancel closure directly (bypassing the
	// floatingCloseOnce guard used by the normal teardown paths) must remain
	// safe: both context.CancelFunc and the AfterFunc stop function tolerate
	// repeated calls.
	...
	case <-openCtx.Done():
	case <-time.After(time.Second):
		t.Fatal("pane cancel closure did not cancel the PTY-owning context")
	}
	select {
	case <-pty.closed:
	case <-time.After(time.Second):
		t.Fatal("cancelling the PTY-owning context did not unblock and close the floating PTY")
	}
	d.sessWg.Wait()
}

func TestFloatingFailedOpenLeavesSlotUninitializedForRetry(t *testing.T) {
	pty := newContextAwareFloatingPTY()
	factory := &contextAwareFloatingFactory{pty: pty, opened: make(chan context.Context, 1), err: context.Canceled}
	d := newTestDaemon(t, factory, stubClock{})
	tb := newFloatingTestTab(t)
	sess := &session{name: "work", tabs: []*tab{tb}, ctx: t.Context()}
	tb.mu.Lock()
	...
	<-factory.opened
	tb.mu.Lock()
	require.Equal(t, floatingUninitialized, tb.floating.state, "a failed Open must clear the warming slot rather than leaving it stuck")
	require.Nil(t, tb.floating.pane)
	next := tb.beginFloatingWarmLocked(true)
	tb.mu.Unlock()
	require.NotZero(t, next, "an uninitialized slot must accept a new warm launch after a failed Open")
}
