...
func (d *Daemon) enterPicker(sess *session, ac *attachedClient) {
	d.enterPickerForClient(sess, ac, nil)
}
...
// effect to carry the snapshot send).
func (d *Daemon) enterPickerForClient(sess *session, ac *attachedClient, effect *attachmentEffect) {
	if pickerClientEnabled(ac) && effect != nil {
		...
		d.openPickerClientForAttachment(ac, effect, interaction)
		return
	}
