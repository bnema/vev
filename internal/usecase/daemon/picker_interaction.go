...
// across the send — same discipline as sendPaletteInventoryDemand. A nil
// effect skips the send while the overlay state still transitions; those
// paths coincide with attachment teardown where the client-side loop is
// cancelled independently.
func (d *Daemon) openPickerClientForAttachment(ac *attachedClient, effect *attachmentEffect, interaction uint64) {
	...
	ac.overlays.pickerMu.Unlock()
	d.publishPickerClientSnapshot(ac, effect, interaction)
}
...
// Callers hold no locks across the send.
func (d *Daemon) publishPickerClientSnapshot(ac *attachedClient, effect *attachmentEffect, interaction uint64) {
...
// ordered — same discipline as sendPaletteInventoryDemand close demands.
func (d *Daemon) closePickerClientForAttachment(ac *attachedClient, effect *attachmentEffect, interaction uint64) {
