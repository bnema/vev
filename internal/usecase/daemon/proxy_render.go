...
// attachment lease. It repaints local chrome after the remote content resize
// has been sent by the proxy's serialized sender.
...
func (p *proxySession) resize(size domain.Size) bool {
	if p == nil || !size.Valid() {
		return false
	}
	...
	return p.sendGeneration(generation, ports.Frame{
