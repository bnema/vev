...
func TestOutputAckGapForcesFullStateRepaint(t *testing.T) {
	...
	ac.nextStateNum = 5
	require.True(t, ac.shouldResetOutputState(false), "client ack gap should force dependency-free full repaint")

	f := ac.nextOutputFrameLocked([]byte("full repaint"), ac.shouldResetOutputState(false))
	...
	require.False(t, ac.shouldResetOutputState(false), "acked current state can resume incremental output")
	incremental := ac.nextOutputFrameLocked([]byte("incremental"), ac.shouldResetOutputState(false))
