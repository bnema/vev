...
//
// This is a conservative implementation for MVP: wide/ambiguous characters
// are reported as width 2 and rendered as a placeholder
// so that the model cursor always stays aligned with terminal cells.
...
func (s *Screen) putPrintable(r rune) {
	...
	// Conservative normalization: treat wide/emoji as a single-cell '?' placeholder
	// so cursor and damage tracking are always consistent.  The real terminal width
	// is ignored in favour of a safe single-cell replacement.
	if w > 1 {
		r = '?'
		w = 1
	}
	...
	s.record(renderer.Damage{Kind: renderer.DamageText, X: s.Col, Y: s.Row, Width: w, Height: 1, Count: 1})
	s.Col += w
}
...
func (s *Screen) insertChars(n int) {
	...
	s.record(renderer.Damage{Kind: renderer.DamageText, X: s.Col, Y: s.Row, Width: w - s.Col, Height: 1, Count: 1})
}
...
func (s *Screen) clearLineMode(mode int) {
	...
	case 1:
		...
		s.record(renderer.Damage{Kind: renderer.DamageClear, X: 0, Y: s.Row, Width: end, Height: 1, Count: 1})
	case 2:
		for x := 0; x < s.Frame.Width; x++ {
			s.Frame.Set(x, s.Row, renderer.BlankCell())
		}
		s.record(renderer.Damage{Kind: renderer.DamageClear, X: 0, Y: s.Row, Width: s.Frame.Width, Height: 1, Count: 1})
	default:
		for x := s.Col; x < s.Frame.Width; x++ {
			s.Frame.Set(x, s.Row, renderer.BlankCell())
		}
		s.record(renderer.Damage{Kind: renderer.DamageClear, X: s.Col, Y: s.Row, Width: s.Frame.Width - s.Col, Height: 1, Count: 1})
	}
}
