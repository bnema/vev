...
QUIC). Direct-remote scenarios use `--remote` with no local daemon and never
...
| `direct-remote-ephemeral@quic` | M2 direct remote QUIC: fresh attach, committed output, stable lifecycle |
| `direct-remote-ephemeral@stdio` | M3 direct remote explicit stdio: same semantics, no QUIC listener |
...
| `client-picker-navigate@quic` (topology `local`) | M1: client-owned picker presented by the client, searched commit, cancel without a route change |
| `client-picker-navigate@quic` (topology `direct`) | M2: same over the remote serving daemon, no local daemon involved |
