...
# Baseline matrix: local IPC, direct remote (QUIC + explicit stdio, no local
# daemon), and hybrid exact-return (QUIC + stdio). Each scenario asserts
# committed outcomes, not sent keys. navigation_repro.py keeps the original
# hybrid transport regression alongside the table-driven runner.
...
python3 "$repo_root/scripts/remote-picker-harness/acceptance.py" "$local_container" "$remote_container" direct-remote-ephemeral@quic
...
python3 "$repo_root/scripts/remote-picker-harness/acceptance.py" "$local_container" "$remote_container" hybrid-exact-return@quic
...
VEV_ACCEPTANCE_TOPOLOGY=local python3 "$repo_root/scripts/remote-picker-harness/acceptance.py" "$local_container" "$remote_container" client-picker-navigate@quic
VEV_ACCEPTANCE_TOPOLOGY=direct python3 "$repo_root/scripts/remote-picker-harness/acceptance.py" "$local_container" "$remote_container" client-picker-navigate@quic
